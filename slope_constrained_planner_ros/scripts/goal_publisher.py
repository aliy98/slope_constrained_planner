...
class GoalPublisher:
    ...
    def __init__(self):
        ...
        self.pub_goal = rospy.Publisher('/goaltospot', PoseStamped, queue_size=1)
    ...
    def updateCurrentPose(self):
        ...
        if self.fixed_frame is not None:
            self.listener.waitForTransform(self.fixed_frame, ROBOT_FRAME, rospy.Time.now(), rospy.Duration(4.0))
            try:
                now = rospy.Time.now() 
    ...
    def computeAndPublishGoal(self):
        """
        ...
        if self.path is not None and self.current_pose is not None:
            ...
            quat = getQuatFromYaw(yaw_target)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        goal_pub.computeAndPublishGoal()
        rate.sleep()
