/**
 * @file converter.cpp
 * @brief Contains the definition of public member functions of ``Converter`` class
 */
...
Converter::Converter(std::shared_ptr<ob::SE3StateSpace> space) : space_(space) {
...
ob::ScopedState<> Converter::poseRosToOmpl(const geometry_msgs::PoseStamped& pose_ros) const {
  ...
  auto pose_se3 = pose->as<ob::SE3StateSpace::StateType>();
...
nav_msgs::Path Converter::pathOmplToRos(const og::PathGeometric& path_ompl) const {
  ...
  for (size_t i = 0; i < path_ompl.getStateCount(); ++i) {
    ...
    const auto state_ompl = state->as<ob::SE3StateSpace::StateType>();
